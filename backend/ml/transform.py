...
def transform(human_face):
    ...
    human_face.save(human_face_path)

    cat_num = random.rand(0, len([name for name in os.listdir(CAT_DIR) if os.path.isfile(os.path.join(CAT_DIR, name))]) - 1)
    ...
    return True, "/backend/output/movie.mp4"